...
Radio = Norm[NA - NC]/
   2;  (*Radio es la mitad de la distancia entre NA y NC*)
...
theta = -(Pi - ((ANGPARTIDA*
         Pi/180) + ((ANGFINAL*Pi/180) - (ANGPARTIDA*Pi/180))*
        Pi*(10 (t/tf)^3 - 15 (t/tf)^4 + 6 (t/tf)^5)));

(*Parametrizando la semicircunferencia en un plano paralelo*)
x = Centro[[1]] + Radio*Cos[theta]*vectorAB[[1]]/Norm[vectorAB] + 
   Radio*Sin[theta]*vectorAD[[1]]/Norm[vectorAD];
y = Centro[[2]] + Radio*Cos[theta]*vectorAB[[2]]/Norm[vectorAB] + 
   Radio*Sin[theta]*vectorAD[[2]]/Norm[vectorAD];
z = Centro[[3]] + Radio*Cos[theta]*vectorAB[[3]]/Norm[vectorAB] + 
   Radio*Sin[theta]*vectorAD[[3]]/Norm[vectorAD];

(*Generar puntos de la semicircunferencia*)
puntosSemicirc = Table[{x, y, z}, {t, 0, tf, tf/50}];

(*Línea de la semicircunferencia con etiquetas de punto inicial y \
final*)
lineaSemicirc = Line[puntosSemicirc];

(*Añadir etiquetas de punto inicial (NA),final (NC),y punto de inicio \
...
imagen = 
  Graphics3D[{Pink, eslabon1, Yellow, eslabon2, Cyan, eslabon3, Red, 
    plano, Black, lineaSemicirc}, ejes, rango, letreros, origen, 
   pov];
...
Plot[theta, {t, 0, tf}, AxesLabel -> {"t", "Evolución del Perfil Quíntico"}, 
...
Plot[velocidadPerfil, {t, 0, tf}, 
 AxesLabel -> {"t", "Velocidad del Perfil Quíntico"}, 
 PlotStyle -> Red, AxesStyle -> Directive[Thick, Black]]

(*Gráfica de Aceleración del Perfil Quíntico*)
 Plot[\[Theta]pp[1] (180/\[Pi]), {t, 0, tf}, 
 AxesLabel -> {"t", "Aceleración del Perfil Quíntico (grados/segundo^2)"}, 
 PlotStyle -> Blue, AxesStyle -> Directive[Thick, Black]]
