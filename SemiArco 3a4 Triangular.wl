...
theta = Piecewise[{{(2*qf/tf^2)*t^2, 
      t < tf/2}, {(-2*qf/tf^2)*t^2 + (4*qf/tf)*t - qf, t >= tf/2}}] + 
   ANGPARTIDA;

(*Parametrizando la semicircunferencia en un plano paralelo*)
x = Centro[[1]] + 
   Radio*Cos[theta*Degree]*vectorAB[[1]]/Norm[vectorAB] + 
   Radio*Sin[theta*Degree]*vectorAD[[1]]/Norm[vectorAD];
y = Centro[[2]] + 
   Radio*Cos[theta*Degree]*vectorAB[[2]]/Norm[vectorAB] + 
   Radio*Sin[theta*Degree]*vectorAD[[2]]/Norm[vectorAD];
z = Centro[[3]] + 
   Radio*Cos[theta*Degree]*vectorAB[[3]]/Norm[vectorAB] + 
   Radio*Sin[theta*Degree]*vectorAD[[3]]/Norm[vectorAD];

(*Generar puntos de la semicircunferencia*)
puntosSemicirc = 
  Table[{x, y, z} /. t -> tiempo, {tiempo, 0, tf, tf/50}];
...
Plot[theta, {t, 0, tf}, 
 AxesLabel -> {"t", "Evolución del Perfil Triangular"}, 
...
Plot[\[Theta]pp[1] (180/\[Pi]), {t, 0, tf}, 
 AxesLabel -> {"t", 
   "Aceleración del Perfil Triangular (grados/segundo^2)"}, 
 PlotStyle -> Blue, AxesStyle -> Directive[Thick, Black]]
